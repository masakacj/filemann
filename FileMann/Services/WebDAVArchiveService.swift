...
final class WebDAVArchiveService {
    ...
    init(settings: SMBSettings, password: String) throws {
        ...
        guard let url = URL(string: settings.normalizedWebDAVBaseURL),
    ...
    func testConnection() async throws {
        ...
        _ = try await propfind(path: settings.normalizedWebDAVDirectory, depth: "0")
    }
    ...
    func finalizeBackgroundUpload(
        ...
        if await remoteFileSize(atPath: finalPath) != nil {
            throw WebDAVArchiveError.remoteConflict((finalPath as NSString).lastPathComponent)
        }

        var request = URLRequest(url: remoteURL(for: partialPath))
        request.httpMethod = "MOVE"
        request.setValue(remoteURL(for: finalPath).absoluteString, forHTTPHeaderField: "Destination")
        ...
        return ArchiveResult(remoteSize: expectedBytes, remotePath: normalize(finalPath))
    }
    ...
    func verifyManifest(_ tasks: [ArchiveTask]) async -> BatchVerification {
        ...
        for task in expected {
            let path = task.remoteVerifiedPath ?? desiredRemotePath(for: task)
    ...
    func remoteFileSize(atPath path: String) async -> Int64? {
        ...
        do {
            ...
            let parser = WebDAVMultiStatusParser(data: data)
            return try parser.parse().first(where: { !$0.isCollection })?.contentLength
        } catch {
            return nil
        }
    }
    ...
    private func ensureDirectory(_ path: String) async throws {
        ...
        for component in normalized.split(separator: "/").map(String.init) {
            ...
            guard [200, 201, 204, 405].contains(http.statusCode) else {
                throw WebDAVArchiveError.unexpectedStatus(http.statusCode)
            }
        }
    }
    ...
    private func remoteURL(for path: String) -> URL {
        var url = baseURL
        for component in normalize(path).split(separator: "/").map(String.init) {
    ...
    private func relativePath(fromHref href: String) -> String? {
        ...
        if let absolute = URL(string: href), absolute.scheme != nil {
            ...
            rawPath = URL(string: href, relativeTo: baseURL)?.path ?? href
        }

        let decoded = rawPath.removingPercentEncoding ?? rawPath
        let root = baseURL.path.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        let value = decoded.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        ...
        return normalize(String(value.dropFirst(root.count + 1)))
    }

    private func isImmediateChild(_ child: String, of parent: String) -> Bool {
        ...
        let remainder = String(c.dropFirst(p.count + 1))
        return !remainder.isEmpty && !remainder.contains("/")
    }

    private func applyAuthorization(to request: inout URLRequest) {
        guard !settings.webDAVUsername.isEmpty else { return }
        let token = Data("\(settings.webDAVUsername):\(password)".utf8).base64EncodedString()
...
private final class WebDAVMultiStatusParser: NSObject, XMLParserDelegate {
    ...
    func parse() throws -> [WebDAVParsedEntry] {
        ...
        guard parser.parse() else {
            throw parser.parserError ?? parseError ?? WebDAVArchiveError.invalidResponse
        }
        return entries
    }
